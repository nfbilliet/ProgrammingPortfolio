...
the positions of all occurrences of 'U' in "AUGCUUCAGAAAGGUCUUACG" are 2, 5, 6, 15, 17, and 18). 
...
#Find all overlapping substring position in a larger string 

import re 

def readInp(fname):
    with open(fname, "r") as myFile:
        DNAstring, DNAmotif = myFile.read().splitlines()
    return (DNAstring, DNAmotif)

def findMotif(DNAstring, DNAmotif):
    #Lookahead assertion "(?=...)" must be used in the regex pattern to force it to find overlapping occurences
    #String indexing starts at 0 and desired output adds 1 to all string indices
    matchPosition = [str(match.start()+1) for match in re.finditer("(?="+DNAmotif+")", DNAstring)]
    return " ".join(matchPosition)

inp= readInp("rosalind_subs.txt")
print(findMotif(inp[0], inp[1]))